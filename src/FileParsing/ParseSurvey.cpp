...
int ParseSurvey::FindSynchronizedAUXIndex(double querytime, int from_idx){
    ...
    if(debug) cout << "found at: " << from_idx << ", with " << timings[from_idx] << endl;
...
ParseFeatureTrackFile ParseSurvey::LoadVisualFeatureTracks(Camera& _cam, int& index){
    ...
    while(PFT.time<=0) {
        if(!PFT.Exists(PFT.siftfile)) {
            ...
            if(nonexist>60000) {
                if(found) return PFT;
                cout << "SurveyOptimizer::LoadVisualFeatureTracks() Error. There are no feature tracking files. Check directory: " << PFT.siftfile << endl;
                exit(-1);
            }
        }
        if(debug) cout << "Empty PFT file at " << index  << ". loading next." << endl;
        PFT.Next(++index);
    }
    cout.precision(5);
