...
//--------------------------------------------------------------------------------------------------
// function to interpolate a multi-channel image plane for (x,y)
// --------------------------------------------------------------------------------------------------
template <class T1,class T2>
        ...
        for(j=0;j<width;j++)
            for(k=0;k<nChannels;k++)
                for(l=-fsize;l<=fsize;l++) {
                    ...
                    int srcidx = (i*width+j)*nChannels+k;
                    int dstidx = (i*width+jj)*nChannels+k;
                    pDstImage[srcidx] += pSrcImage[dstidx]*w;
                }
}
...
// pWarpIm2 has to be allocated beforehand
//it uses im1 to fill in spots where im2 wouldn't otherwise be warped. (I'll probably want to change this to
// indicate `?', rathat than bias the model to be close to the baseline.
//------------------------------------------------------------------------------------------------------------
template <class T1,class T2>
