...
void MultiCascade::WeightedAlignment(){
    ...
    while(apc > 0.01 && aoc > 0.002) {
        for(int i=0; i<dates.size(); i++){
            std::cout << "    weighted optimization of " << dates[i] << " at " << i << std::endl;
            int tidx = man.GetOpenMachine();
            ws[tidx]->Setup(&poses, &landmarks, &lpd_rerror, &(originPOR[i]), &(cached_landmarks[i]), &(forwardLMap[i]), &lpdi, i);
            ws[tidx]->toLogPoseChange(&ac[i].first, &ac[i].second);
            ws[tidx]->SetWeight(2.0/3);
            ws[tidx]->SessionDates(dates);
            ws[tidx]->FilterBad(false);
            man.RunMachine(tidx);
        }
        man.WaitForMachine(true);
        apc = 0;
...
void MultiCascade::ContinueUntilAligned(){
    ...
    int iteration = 0;
    std::vector<std::string> datessubset;
    
    
    datessubset.push_back(dates[0]);
    for(int i=0; i<dates.size(); i++){
        std::cout << "    the initial optimization of " << dates[i] << " at " << i << std::endl;
        datessubset.push_back(dates[i]);
        int tidx = man.GetOpenMachine();
        ws[tidx]->Setup(&poses, &landmarks, &lpd_rerror, &(originPOR[i]), &(cached_landmarks[i]), &(forwardLMap[i]), &lpdi, i);
        ws[tidx]->toLogPoseChange(apc, aoc);
        ws[tidx]->SessionDates(datessubset);
        ws[tidx]->FilterBad(false);
        man.RunMachine(tidx);
        man.WaitForMachine(true);
        std::cout << ""
    ...
    for(int i=0; i<dates.size(); i++){
        ...
        for(int j=0; j<datessubset.size(); j++){
            std::cout << datessubset[j] << ",";
            int tidx = man.GetOpenMachine();
            ws[tidx]->Setup(&poses, &landmarks, &lpd_rerror, &(originPOR[i]), &(cached_landmarks[i]), &(forwardLMap[i]), &lpdi, i);
            ws[tidx]->SessionDates(datessubset);
            ws[tidx]->FilterBad(filterbad);
    ...
    while(count > 0){
        ...
        if(filterbad)
            count--;
        std::cout << "ITERATION " << iteration++ << " reached ISC filter stage? " << filterbad << ". Changes: " << averagechange << " -> " << last << std::endl;
        averagechange = last;
