...
void MultiCascade::AddSessionToLocalize(std::string date){
    std::string lmap_dir = _map_dir + "../localized_maps/";
    //this implementation of optimization for session localization is inefficient, but it was an easy way to add.
    ParseOptimizationResults datePOR(lmap_dir, date);
    ...
    LPDInterface lint;
    std::cout <<  date << ": ";
    int nloaded = lint.LoadLocalizations(lmap_dir, date);
    lpdi.push_back(lint);
    permerr.push_back(vector<double>(lint.localizations.size(),0));
    
    EvaluateSLAM ESlam(_cam, date, lmap_dir);
    rerrs.push_back(ESlam.LoadRerrorFile());
    double avg = ESlam.GetAverageRerror(rerrs[dates.size()-1]);
    AverageRerror.push_back(avg);
    
    std::vector<LandmarkTrack> clset;
    cached_landmarks.push_back(clset);
    
    HopcountLog hlog(lmap_dir);
    vector<double> rerror_set = hlog.LoadPriorRerror(date, nloaded);
    ...
    for(int j=0; j<POR[survey].boat.size(); j++){
        rerrs[survey][j] = rerrs[survey][j]*update_mult_factor;
        if(rerrs[survey][j] < 0) {
            ...
            exit(1);
        } else if (rerrs[survey][j] < 6) { //this occurs at places in the rerr vector that are zero.
            rerrs[survey][j] = AverageRerror[survey]*update_mult_factor;
        }
    }
    heights = std::vector<double>(dates.size(), 0);
    
    cache_set = survey; //update the parent class variable
    rfFG->ChangeLandmarkSet(survey);
    ...
    for(int i=0; i<POR[survey].boat.size(); i++) {
        ParseFeatureTrackFile pftf = ParseFeatureTrackFile::LoadFTF(_cam, _pftbase + dates[survey], oPOR.ftfilenos[i]);
        std::vector<gtsam::Point3> p3d = oPOR.GetSubsetOf3DPoints(pftf.ids);
